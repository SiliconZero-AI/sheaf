...
- 🪟 **多窗口并排改**：右键稿件选「在新窗口打开」，两篇拖到屏幕两边同时编辑，也能跟浏览器、Word 并排；`Ctrl+Shift+N` 开一个空白新窗口。同一篇永远只有一个窗口，关掉其中一个不影响别的，关光了才退出
...
- 🔎 **正文能放大缩小**：`Ctrl` + `+` / `-` 调大小、`Ctrl+0` 回到原始大小，按住 `Ctrl` 滚滚轮也行——稿子密、人离屏幕远的时候用得上，比例关掉重开还在
- 🖼️ **图和流程图点开全屏看**：鼠标移上去右上角冒出按钮，点开铺满窗口，滚轮缩放、按住拖着看、`Esc` 退回来。宽流程图被压到正文栏那么窄、字全挤成一团时，就靠它
- 📎 **单开一篇也不裂图**：从资源管理器双击 `.md`，或用「打开单篇」，正文里的相对路径图片会从稿件所在目录读取；保存后仍是原来的相对路径
- ✏️ **左栏直接重命名**：右键稿件选「重命名」，或选中后按 `F2`；输入框只改主文件名，`.md` / `.markdown` 扩展名自动保留
- 🗑️ **左栏直接删稿子**：右键任意一篇选「删除」（或选中后按 Del），删之前先问一句，删掉的进回收站；**删错了紧接着按 Ctrl+Z 就撤回来**，不用为了删一个测试文件去开资源管理器
- 🌗 **深浅双主题**：默认跟系统走，顶栏一键锁定
- 🌐 **中英双语界面**：一键切换，即时生效不刷新
- ⬆️ **自己升级自己**：有新版会在启动时提示你，框里列出这一版改了什么，点一下就下载安装并重开，工作区和每篇读到哪都不会丢
- 🪟 **Windows 安装包**：双击装、开始菜单直接开，零外链断网可用，关联 `.md` 文件（双击就用 Sheaf 打开），关程序重开自动恢复上次的文件夹和稿子——这份记忆存在应用数据目录之外，升级时勾了「删除应用数据」也带不走
...
```bash
npm test           # 文件、图片、同步、更新等关键路径的回归测试
```
